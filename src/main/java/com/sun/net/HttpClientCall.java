...
public class HttpClientCall {
	private static final Logger logger = LogManager.getLogger(HttpClientCall.class);
	...
	public String sendHttpPostRequest(String url, Map<String, String> params) {
		...
		try {
			...
			logger.error("---------------接口异常：url=" + url + ",reqParam="+ JSON.toJSONString(params), e);
		} finally {
			if (reader != null) {
				try {
					reader.close();
	...
	/**
	 * 发送 aop请求获取接口响应数据 Description:<br>
	 * 
	 * @param url
	 *            请求路径
	 * @param params
	 ...
	 */
	@SuppressWarnings("finally")
	public Map<String,String> sendAopHttpPostRequest(String url, Map<String, String> params)throws Exception {
		...
		try {
			// 执行postMethod
			int statusCode = client.executeMethod(postMethod);
			logger.info("aop查询接口响应状态码：" + statusCode);
			...
			logger.error("aop查询接口异常：url=" + url + ",reqParam="
			...
			postMethod.releaseConnection();
			map.put("info", respContent.toString());
			return map;
		}
	}
